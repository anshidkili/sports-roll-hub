...
export function CreateSportForm({ onSuccess, onCancel }: CreateSportFormProps) {
  ...
  const validateForm = (): string | null => {
    ...
    if (formData.registrationDeadline && formData.registrationDeadline <= new Date()) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: `${formData.type === 'game' ? 'Game' : 'Athletic event'} "${formData.name}" created successfully!`,
      });
      ...
      toast({
        title: 'Error',
        description: 'Failed to create sport event. Please try again.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-2xl">Create Sports Event</CardTitle>
        </div>
        <CardDescription>
